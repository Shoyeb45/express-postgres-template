...
const getReadableErrorMessage = (code, path, message, received, expected) => {
    ...
    switch (code) {
        ...
        case 'too_small':
            ...
            if (message.includes('Number')) {
                const minMatch = message.match(/greater than or equal to (\d+)/);
        ...
        case 'unrecognized_keys':
            const keys = message.match(/Unrecognized key\(s\) in object: (.+)/)?.[1];
            return keys
...
const validator = (schema, source) => {
    return (req, _res, next) => {
        ...
        if (!result.success) {
            const errors = JSON.parse(String(result.error));
            const validationErrors = errors.map((err) => ({
                field: formatFieldName(err.path),
                message: getReadableErrorMessage(err.code, err.path, err.message, 'received' in err ? err.received : undefined, 'expected' in err ? err.expected : undefined),
                ...(!config_js_1.isProduction && {
            ...
            return next(new ApiError_1.BadRequestError(mainMessage));
        }
        next();
    };
};
