    ...
    static handle(err, res) {
        switch (err.type) {
            ...
            case ErrorType.UNAUTHORIZED:
                return new ApiResponse_js_1.AuthFailureResponse(err.message).send(res);
            case ErrorType.ACCESS_TOKEN:
                return new ApiResponse_js_1.AccessTokenErrorResponse(err.message).send(res);
            case ErrorType.INTERNAL:
                return new ApiResponse_js_1.InternalErrorResponse(err.message).send(res);
            case ErrorType.NOT_FOUND:
            case ErrorType.NO_ENTRY:
            case ErrorType.NO_DATA:
                return new ApiResponse_js_1.NotFoundResponse(err.message).send(res);
            case ErrorType.BAD_REQUEST:
                return new ApiResponse_js_1.BadRequestResponse(err.message).send(res);
            case ErrorType.FORBIDDEN:
                return new ApiResponse_js_1.ForbiddenResponse(err.message).send(res);
            default: {
                let message = err.message;
                if (config_js_1.isProduction) {
                    message = 'Something wrong happened.';
                }
                return new ApiResponse_js_1.InternalErrorResponse(message).send(res);
            }
        }
    }
}
